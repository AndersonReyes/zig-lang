...
pub const Signal = struct {
    ...
    /// function that evaluates the signal values
    function: *const fn (f64) f64,
    ...
    pub fn init(
        ...
        return .{ .frequency = frequency, .amplitude = amplitude, .phase = phase, .function = function };
    }
    ...
    pub fn initCos(
        ...
        return .{ .frequency = frequency, .amplitude = amplitude, .phase = phase, .function = wrapper.cos };
    }
    ...
    pub fn initSin(
        ...
        return .{ .frequency = frequency, .amplitude = amplitude, .phase = phase, .function = wrapper.sin };
    }

    fn evaluate(self: Signal, n: comptime_int, time: [n]f64) [n]f64 {
        var out: [n]f64 = .{0.0} ** n;

        for (0..n) |k| {
            const r: f64 = self.phase + (self.frequency * std.math.pi * 2.0 * time[k]);
            out[k] = self.function(r) * self.amplitude;
        }
        return out;
    }

    /// evaluate signal at vector of timestamps. Uses the vector operations to evaluate multiple timestamps at once.
    pub fn evaluateMany(signals: []const Signal, n: comptime_int, time: [n]f64) [n]f64 {
        var sum: [n]f64 = .{0.0} ** n;

        for (signals) |s| {
            const ys = evaluate(s, n, time);
            for (0..n) |i| {
                sum[i] += ys[i];
            }
        }
